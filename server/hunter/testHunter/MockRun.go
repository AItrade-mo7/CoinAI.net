...
func (_this *MockObj) MockRun() BillingType {
	...
	stopReason := "norm"
	_this.CAPRecord = make(map[int64]string)
	for i, Kdata := range _this.RunKdataList {
		if i%500 == 0 && _this.RunFull {
		...
		// 开始执行分析交易
		_this.Analy()
		_this.CAPRecord[Kdata.TimeUnix] = _this.NowVirtualPosition.CAP_EMA

		if len(FormatEnd)-MaxLen > 0 {
			FormatEnd = FormatEnd[len(FormatEnd)-MaxLen:]
		}

		if mCount.Le(_this.NowVirtualPosition.NowUplRatio, "-45") < 0 && !_this.RunFull {
			global.Log.Info("爆仓！", zap.String(_this.Billing.MockName, Kdata.TimeStr))
			stopReason = "爆仓"
			break
		}

		if mCount.Le(_this.NowVirtualPosition.Money, MinMoney) < 0 && !_this.RunFull {
		...
		if mCount.Le(_this.NowVirtualPosition.FloatMoney, threshold) < 0 && !_this.RunFull {
